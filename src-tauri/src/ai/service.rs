...
pub async fn ai_request(
    ...
    if cred.provider == "openai-codex" {
        ...
        // ChatGPT subscriptions route through the Codex Responses API.
        // Confirmed-current models (learn.chatgpt.com/docs/models, July
        // 2026): gpt-5.6-sol/terra/luna, gpt-5.5, gpt-5.4/-mini. Slugs
        // deprecate fast (gpt-5, gpt-4o, gpt-4o-mini all previously
        // failed here) — gpt-5.6-terra is the current balanced default.
        let model = cred.model.as_deref().unwrap_or("gpt-5.6-terra");
